...
if __name__ == '__main__':
  ...
  for person in prefs2:
    topFifty = {}
    mostSimilar = topMatches(prefs, person, 50, sim_pearson)

    for (x,y) in mostSimilar:
      topFifty[y] = prefs[y]

    topFifty[person] = prefs[person]
    recommendedRanks = getRecommendedRanks(topFifty, person)
    r1 = compareRec2True(topFifty, person, recommendedRanks)
    rList.append(r1)
    sum1 += r1
